...
[事業者宛](ssp-vendor.md) — まずこちら。robots.txt は取得を許可しているが、画面が
JavaScript で構成されており、データ取得先が Disallow 対象のスクリプト内にあるため
着手できない。1件の回答で18県分の見通しが立つ。

[議会宛の雛形](ssp-assembly.md) — 事業者から「各議会の同意が必要」と回答があった
場合、または事業者の窓口が不明な場合に、対象18県へ個別に送る。
