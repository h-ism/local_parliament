...
**文面の書き方**：読むのは議会事務局の職員であって、サーバ設定の担当者とは
限らない。robots.txt やパス名を本文に並べても判断材料にはならず、身構えさせる
だけになる。**本文は平易な日本語で事情を説明するに留め、パスの照合や設定の詳細は
末尾の【技術的な補足】に分ける**（事務局から情報システム担当や事業者へ転送する際は
そこだけを渡せばよい）。[千葉](chiba.md) が改訂済みの見本。他の文面は旧い書き方の
ままなので、送る前にこの形に直すこと。

## 0. 既存コーパスへの照会 — **まずこれ**

[地方議会会議録コーパスプロジェクト](local-politics.md) — 許可の依頼ではなく、
二重取得を避けるための照会。先方は**全47議会・2011年4月〜2019年3月**を既に
整備している。収集方針が「取得できる限りの全期間」である以上、この範囲は
除外対象ではなく確実に欲しい範囲であり、同じものを47議会分クロールし直す理由が
ない。和歌山と愛媛には、この照会の結果が出るまで意図的な穴が空いたままになる。

**2019年4月以降の継続予定も必ず尋ねること。** 継続しているなら、いま収集している
範囲まで重複しうる。
...
本文は UTF-8 の HTML で、907件を収集済み（1989-02-27 .. 2025-12-19）。
※ ここには「本文が HTML か PDF か未確認」と書かれていたが、2026-08-27 の収集で
解消した。索引リンクの破損（令和7年6月第6号）を伝える先として、別途 `wakayama.md`
を起こす余地はある。
